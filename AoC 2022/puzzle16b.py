...
def checkValue(vlist,distances,time=26):
    score=0
    time_spent=0
    for i in range(len(vlist)-1):
        dist=0
...
def checkMax(vlist,rest,time=26):
    score=0
    time_spent=0
    for i in range(len(vlist)-1):
        dist=0
...
def heuristicRun(vlist,distances,time):
    ...
    score=0
    while time>=0:
        vlist.sort(key=lambda x: x.calcWeightedScore(time,distances[(current.getName(),x.getName())]))
        next=vlist.pop(-1)
...
for x in complete_graph:
    ...
    if not isIn:
        second.append(x)
start=complete_graph[-1]
path=[start]
print("\n")
print(" ")
bound,best=heuristicRun(second,distances,time)
nextpath=recursive(path,second,time,distances)
print(part1)
print(bound)
for x in best:
    print(x.getName())
